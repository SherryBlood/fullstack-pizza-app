...
$conn->query("
  ...
  SET order_status = CASE 
      WHEN type_delivery = 'self_pickup' AND order_datetime <= NOW() THEN 'wait_customer'
      WHEN type_delivery = 'address_pickup' AND order_datetime <= NOW() THEN 'issue_courier'
      ELSE order_status
  END
...
$result = $conn->query("
  SELECT o.*, b.created_at, c.first_name, c.last_name, c.phone_number, c.address
  FROM orders o
  JOIN baskets b ON o.basket_id = b.basket_id
  JOIN customers c ON o.customer_id = c.customer_id
  WHERE o.order_status IN ('getting_ready', 'wait_customer', 'issue_courier')
...
<body>
  ...
  <main>
    <section class="hero-section-orders-admin">
      <div class="container">
        <h1 class="hero-title hero-title-order">Active orders</h1>
      </div>
    </section>
    ...
    <section class="menu-orders-section-elements" id="menu-orders-section-elements">
      <div class="container orders-container-elements">
        <div class="orders-elements-butt">
          <input 
          class="form-button button search-button search-button-elements" 
          type="button" 
          value="Active" 
          onclick="window.location.href='getting_orders_admin_page.php';">
          <input 
          class="form-button button search-button search-button-elements" 
          type="button" 
          value="Completed" 
          onclick="window.location.href='issued_orders_admin_page.php';">
          <input 
          class="form-button button search-button search-button-elements" 
          type="button" 
          value="Cancelled" 
          onclick="window.location.href='cancelled_orders_admin_page.php';">
          <input 
          class="form-button button search-button search-button-elements" 
          type="button" 
          value="Clear old carts" 
          onclick="window.location.href='clear_old_baskets_admin_page.php';">
        </div>
      </div>
    </section>
        ...
        <div class="container order-admin-container">
          ...
          <div class="order-info-admin">
            ...
            <?php if ($status === 'getting_ready'): ?>
              <form method="post" action="cancel_admin_order.php">
            ...
            <?php if ($status === 'wait_customer' || $status === 'issue_courier'): ?>
              <form method="post" action="issue_admin_order.php">
                <input type="hidden" name="order_id" value="<?= $order['order_id'] ?>">
          ...
          <div class="order-info-client-admin">
            ...
            <?php if ($order['type_delivery'] === 'address_pickup'): ?>
              ...
              $courierRes->close();
              ?>
              <form method="post" action="assign_courier.php">
                <div class="courier-assign-form">
                  <input type="hidden" name="order_id" value="<?= $order['order_id'] ?>">
                  <label for="courier-select-<?= $order['order_id'] ?>">Courier:</label>
                  <select name="courier_id" id="courier-select-<?= $order['order_id'] ?>" required>
                    <option value="">Select a courier</option>
                    <?php
                    $couriersRes = $conn->query("SELECT courier_id, first_name, last_name FROM couriers");
                    while ($courier = $couriersRes->fetch_assoc()):
                      $cId = (int)$courier['courier_id'];
                      $name = htmlspecialchars($courier['first_name'] . ' ' . $courier['last_name']);
                      $selected = ($cId === $assignedCourierId) ? 'selected' : '';
                      ?>
                      <option value="<?= $cId ?>" <?= $selected ?>><?= $name ?></option>
                    <?php endwhile; ?>
                  </select>
                  <button class="form-button button order-courier-change-button" type="submit">Select</button>
                </div>
              </form>
            <?php endif; ?>
