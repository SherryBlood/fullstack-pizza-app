...
<body>
  ...
  <main>
    <section class="hero-section-orders-admin">
      <div class="container">
        <h1 class="hero-title hero-title-order">Completed Orders</h1>
      </div>
    </section>
...
if ($order['order_status'] === 'issued') {
    $stmt = $conn->prepare("SELECT issued_at FROM issued_orders WHERE order_id = ?");
    $stmt->bind_param("i", $order['order_id']);
    $stmt->execute();
    $res = $stmt->get_result();
    if ($res && $row = $res->fetch_assoc()) {
        $issued_at_value = date('Y-m-d\TH:i', strtotime($row['issued_at']));
    }
    $stmt->close();
}
...
<?php endif; ?>
        ...
        <?php if (!empty($issued_at_value)): ?>
    Completed at
    <input class="client-order-datetime" type="datetime-local" value="<?= $issued_at_value ?>" readonly>
  <?php endif; ?>
      </div>
  ...
  <div class="courier-assign-form">
    ...
    <select name="courier_id" id="courier-select-<?= $order['order_id'] ?>" disabled>
