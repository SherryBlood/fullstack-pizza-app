...
if (isset($_GET['search-admin'])) {
    $searchTerm = $conn->real_escape_string($_GET['search-admin']);


    $productsQuery = "SELECT *, 'product' AS type FROM products WHERE name LIKE '%$searchTerm%'";
    $productsResult = $conn->query($productsQuery);


    $setsQuery = "SELECT *, 'set' AS type FROM sets WHERE name LIKE '%$searchTerm%'";
    $setsResult = $conn->query($setsQuery);
} else {
    echo '<script>alert("Нічого не введено для пошуку."); window.history.back();</script>';
    exit;
}
...
<body>
  ...
  <main>
    <section class="hero-section-menu-search-admin">
      <div class="container">
        <h1 class="hero-title hero-title-order">Search</h1>
      </div>
    </section>
    ...
    <section class="menu-pizzas-section" id="menu-pizzas-section">
      <div class="products-container" id="products-container">
        <?php if (($productsResult && $productsResult->num_rows > 0) || ($setsResult && $setsResult->num_rows > 0)): ?>
        ...
        <div class="container menu-pizzas-container">
          ...
          <div class="pizza-price-btns">
            ...
            <div class="pizza-option-button">
              ...
              <button 
                class="form-button button modal-edit-set-btn-open" 
                type="button"
                data-id="<?= $set['set_id'] ?>"
                data-name="<?= htmlspecialchars($set['name']) ?>"
                data-description="<?= htmlspecialchars($set['description']) ?>"
                data-price="<?= $set['price'] ?>">Edit</button>

              <form method="post" action="delete_set.php" onsubmit="return confirm('Are you sure you want to delete this set?');">
                <input type="hidden" name="id" value="<?= $set['set_id'] ?>">
                <button class="form-button button" type="submit">Delete</button>
              </form>
            </div>
  ...
  <div class="backdrop-sets-edit is-hidden">
    <div class="modal modal-sets-edit">
      <button class="form-button modal-sets-edit-btn-close button" type="button">Close</button>
      <h2>Edit Set</h2>
      <form name="modal-form-edit-set" method="post" action="admin_edit_set.php" enctype="multipart/form-data" autocomplete="off">
        <div class="form-field">
          <label class="form-label visually-hidden" for="set-name-edit">Name</label>
          <input 
          class="form-input" 
          type="text" 
          name="set-name-edit" 
        ...
        <div class="form-field">
          <label class="form-label visually-hidden" for="set-description-edit">Description</label>
          <textarea 
          class="form-input form-input-textarea" 
          rows="3" 
          name="set-description-edit" 
          id="set-description-edit"
          placeholder="Description"
          required></textarea>
        </div>
        <div class="form-field">
          <label class="form-label visually-hidden" for="set-price-edit">Price ($)</label>
          <input 
          class="form-input" 
          type="number" 
          name="set-price-edit" 
        ...
        <div class="form-field">
          <label class="form-label visually-hidden" for="set-image-edit">Image</label>
          <input 
          class="form-input" 
          type="file" 
          name="set-image-edit" 
        ...
        <button class="form-button button button-edit-set" type="submit">Save</button>
      </form>
    </div>
  </div>
  ...
  <script src="modal-admin-add-products-sets.js"></script>
  <script src="modal-admin-edit-set.js"></script>
