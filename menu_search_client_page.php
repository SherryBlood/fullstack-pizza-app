...
$searchTerm = '';
if (isset($_GET['search-client'])) {
    $searchTerm = $conn->real_escape_string($_GET['search-client']);

    $productsQuery = "SELECT * FROM products WHERE name LIKE '%$searchTerm%'";
    $productsResult = $conn->query($productsQuery);

    $setsQuery = "SELECT * FROM sets WHERE name LIKE '%$searchTerm%'";
    $setsResult = $conn->query($setsQuery);
} else {
    echo '<script>alert("It is nothing to search."); window.history.back();</script>';
    exit;
}
?>
      ...
      <div class="container menu-pizzas-container">
        ...
        <div class="pizza-price-btns">
          ...
          <div class="pizza-option-button">
            ...
            <?php if ($isCustomerLoggedIn): ?>
            <form method="post" action="add_set_to_basket.php"> 
              <input type="hidden" name="id" value="<?= $set['set_id'] ?>">
              <button class="form-button button" type="submit">Add to Cart</button> 
              </form>
              <?php else: ?>
